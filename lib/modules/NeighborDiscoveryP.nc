...
//when the timer is fired:
    event void beaconTimer.fired() {
        ...
        msg.seq = 0; //sequence number (this way we know what # packet we are sending/recivieng)
        ...
        if (strcmp(p->payload, "Hellooo") == 0) {
            ...
            for (i = 0; i < count; i++) {
                if (neighbors[i] == p->src) {
                    timeouts[i] = TIMEOUT; //if it was already found then reset the TTL, because the jump was uneccesary
                    found = TRUE; 
                    break; 
                }
            }
